...
! Nx would be a count for columns (weird isnt it?)
! Ny would be a count for rows (With good reason)

! for a good, fast, nondivergent test with KM method
! Nx = 20, Ny = 20, delta_t = 0.0001, Re = 1

! I think for the project, we should exclaim we created framework for two methods
! NS is most stable, KM is not stable -> has to do with GMRES increasing 
! less GMRES functions = better rates for this project
! at outlet, major difference in max velocity only 0.1 m/s, 5% max error with KM method in channel flow
! must say because timeframe of project, will be using simpler method due to quickness
...
module program_variables
      ...
      ! there is a bug where if N_x =/= N_y, code breaks
      integer, parameter :: N_x = 150 ! columns - j
      integer, parameter :: N_y = 40 ! rows - i
      
      double precision, parameter :: L_x = 30D0
      double precision, parameter :: L_y = 8D0
      
      double precision, parameter :: delta_t = 0.000001
      double precision, parameter :: Re = 100D0

      double precision, parameter :: u_inlet = Re
      double precision, parameter :: v_inlet = 0


      integer, parameter :: sq_cyl_row_index_start = 18
      integer, parameter :: sq_cyl_col_index_start = 51

      logical, parameter :: kim_moin = .False.
      ...
      integer, parameter :: N_x_sq_cyl = 5
      integer, parameter :: N_y_sq_cyl = 5
      integer, parameter :: sq_cyl_cells = N_x_sq_cyl * N_y_sq_cyl
      !integer, parameter :: sq_cyl_border_cells = 2 * (N_x_sq_cyl + N_y_sq_cyl) - 4
      ! if we exclude the square cylinder corners
      integer, parameter :: sq_cyl_border_cells = 2 * (N_x_sq_cyl + N_y_sq_cyl) - 8
      integer, parameter :: sq_cyl_int_cells = (N_x_sq_cyl-2) * (N_y_sq_cyl-2)
      integer, parameter :: sq_cyl_cor_cells = 4



      ! count the number of non_zero_variables in Ax = b in A
      ...
      integer, parameter :: int_eq = 5 * ((N_x * N_y) - sq_cyl_cells)

      integer, parameter :: sq_cyl_wall_eq = 2 * sq_cyl_border_cells
      integer, parameter :: sq_cyl_int_eq = sq_cyl_int_cells
      integer, parameter :: sq_cyl_cor_eq = 5 * sq_cyl_cor_cells

      integer, parameter :: nz_num = corner_eq + top_and_bottom_eq + inlet_eq + outlet_eq + int_eq + sq_cyl_wall_eq + & 
            sq_cyl_int_eq + sq_cyl_cor_eq
      integer, parameter :: inlet_eq_u_tmp = N_y ! u_inlet declared directly on u* at inlet
      integer, parameter :: outlet_eq_u_tmp = 3 * N_y ! need 3 to create 2nd order approx for du/dx = 0, N_x outlet cells total
...
program square_cylinder_v1
      ...
      write(6,*) N_x_sq_cyl
      write(6,*) N_y_sq_cyl
      write(6,*) sq_cyl_cells
      write(6,*) sq_cyl_border_cells
      write(6,*) sq_cyl_int_cells
      write(6,*) sq_cyl_wall_eq
      write(6,*) sq_cyl_int_eq 
      ...
      if(kim_moin.eqv..True.) then
            ...
            !do i = 1, nz_num_u_tmp
            !      write(6,*) i, a_spr_u_tmp(i), r_spr_u_tmp(i), c_spr_u_tmp(i)
            !end do
      
            !do i = 1, nz_num_v_tmp
            !      write(6,*) i, a_spr_v_tmp(i), r_spr_v_tmp(i), c_spr_v_tmp(i)
            !end do
      ...
      do while (t_sim.lt.0.5)
            ...
            if (kim_moin.eqv..True.) then
                  ...
                  if(t_step.gt.t_step_trigger) then
                        ...
                        ! goal for kim and moin (create sparse matrix b in Ax = b for u* and v*)
                        call sparse_b_km(u, v, u_h, v_h, p, u_nm1, v_nm1, u_h_nm1, v_h_nm1, b_spr_u_tmp, b_spr_v_tmp)

                        !do i = 1, nz_num_u_tmp
                        !      write(6,*) i, b_spr_u_tmp(i)
                        !end do

                        !write(6,*) " "
                        !do i = 1, nz_num_v_tmp
                        !      write(6,*) i, b_spr_v_tmp(i)
                        !end do
            ...
            if(skip_to_k_m.eqv..False.) then
                  ...
                  call u_tmp_sq_cyl(u_tmp, p)
                  call v_tmp_sq_cyl(v_tmp, p)
            ...
            ! we will place the output to paraview here
            ...
            !write(6,*) "u temp"
            !do i = 1, N_y
            !      write(6,*) u_tmp(i,1:N_x+1) 
            !end do
            !write(6,*) "v temp"
            !do i = 1, N_y+1
            !      write(6,*) v_tmp(i,1:N_x) 
            !end do
            !!end if
...
end program square_cylinder_v1
...
subroutine write_output_to_file(t_step, x_h, y_h, u_h, v_h, P)
      ...
      do i = 1, N_y
            do j = 1, N_x
                  !write(6,*) x_h(j), y_h(i), u_h(i,j), v_h(i,j), P(i,j)
      ...
      201 FORMAT("flow2DIncompCyl_", I10.10,".dat")
...
subroutine update_vel(P, u_tmp, v_tmp, u, v)
      ...
      do i = 1, N_y
            do j = 1, N_x+1
                  ...
                  if(i.ge.19.and.i.le.21.and.j.ge.52.and.j.le.55) then
                        u(i,j) = 0
                  else
      ...
      do i = 1, N_y + 1
            do j = 1, N_x
                  ...
                  if(i.ge.19.and.i.le.22.and.j.ge.52.and.j.le.54) then
                        v(i,j) = 0
                  else
...
subroutine verbose_poisson_matrix(on, r_spr, c_spr, a_spr, b_spr)
      ...
      if(on.eqv..True.) then
            ...
            do i = 1, N_eq
                  !write(6,101) A_full(i,:), b_spr(i), i, count(A_full(i,:)/=0)
                  write(6,*) A_full(i,:), b_spr(i), i, count(A_full(i,:)/=0)
            end do
      
      end if

      !101 format(30(f7.2,1x),f10.2, 1x, I2.2, 1x, I1.1)
...
subroutine sparse_vectors(spr_index, r_spr, c_spr, a_spr)
      ...
      do i = 0, N_y + 1
            do j = 0, N_x + 1
                  ...
                  ! bottom cylinder wall (excludes corners)
                  if(i.eq.18.and.j.gt.51.and.j.lt.55) then
                        
                        call sparse_build(row_cnt, spr_index(i-1,j), nz_index, r_spr, c_spr)
                        a_spr(nz_index) = 1

                        !write(6,*) "bot cyl wall ext", i-1, j, a_spr(nz_index)
                        
                        call sparse_build(row_cnt, spr_index(i,j), nz_index, r_spr, c_spr)
                        a_spr(nz_index) = -1
                        
                        !write(6,*) "bot cyl wall int", i, j, a_spr(nz_index)
                        
                        cycle

                  end if

                  ! top cylinder wall (excludes corners)
                  if(i.eq.22.and.j.gt.51.and.j.lt.55) then

                        call sparse_build(row_cnt, spr_index(i,j), nz_index, r_spr, c_spr)
                        a_spr(nz_index) = -1
                        !write(6,*) "top cyl wall int", i, j, a_spr(nz_index)

                        call sparse_build(row_cnt, spr_index(i+1,j), nz_index, r_spr, c_spr)
                        a_spr(nz_index) = 1
                        !write(6,*) "top cyl wall ext", i+1, j, a_spr(nz_index)

                        cycle

                  end if

                  ! left cylinder wall (excludes corners)
                  if(j.eq.51.and.i.gt.18.and.i.lt.22) then

                        call sparse_build(row_cnt, spr_index(i,j-1), nz_index, r_spr, c_spr)
                        a_spr(nz_index) = 1
                        !write(6,*) "left cyl wall ext", i, j-1, a_spr(nz_index)
                        
                        call sparse_build(row_cnt, spr_index(i,j), nz_index, r_spr, c_spr)
                        a_spr(nz_index) = -1
                        !write(6,*) "left cyl wall int", i, j, a_spr(nz_index)  

                        cycle

                  end if

                  ! right cylinder wall (excludes corners)
                  if (j.eq.55.and.i.gt.18.and.i.lt.22) then
                      
                        call sparse_build(row_cnt, spr_index(i,j), nz_index, r_spr, c_spr)
                        a_spr(nz_index) = -1
                        !write(6,*) "right cyl wall int", i, j, a_spr(nz_index)
                        
                        call sparse_build(row_cnt, spr_index(i,j+1), nz_index, r_spr, c_spr)
                        a_spr(nz_index) = 1
                        !write(6,*) "right cyl wall ext", i, j+1, a_spr(nz_index)
                  ...
                  ! for square cylinder
                  if (i.gt.18.and.i.lt.22.and.j.gt.51.and.j.lt.55) then

                        call sparse_build(row_cnt, spr_index(i,j), nz_index, r_spr, c_spr)
                        a_spr(nz_index) = 1
                        !write(6,*) "cyl interior", i, j, a_spr(nz_index)
                        cycle
            ...
            do j = 0, N_x + 1
                  ...
                  if(j.ge.1.and.j.le.N_x.and.i.eq.0) then
                       ...
                       !call sparse_build(row_cnt, spr_index(i+1,j), nz_index, r_spr, c_spr)
                        ...
                        !b_spr(row_cnt) = (1D0/delta_t) * v_tmp(i+1,j)
                        b_spr(row_cnt) = 0
                  ...
                  if(j.ge.1.and.j.le.N_x.and.i.eq.N_y+1) then
                       ...
                       !call sparse_build(row_cnt, spr_index(i,j), nz_index, r_spr, c_spr)
                        ...
                        !b_spr(row_cnt) =  (1D0/delta_t) * v_tmp(i,j)
                        b_spr(row_cnt) = 0
                       cycle 
                  end if
                  ...
                  if(j.eq.0.and.i.ge.1.and.i.le.N_y) then
                        ...
                        !call sparse_build(row_cnt, spr_index(i,j), nz_index, r_spr, c_spr)
                        !call sparse_build(row_cnt, spr_index(i,j+1), nz_index, r_spr, c_spr)
                        !b_spr(row_cnt) = (1D0/delta_t) * (u_tmp(i,j+1) - u_inlet)
                        b_spr(row_cnt) = 0
                        cycle      
                  end if
                  ...
                  if(j.eq.N_x+1.and.i.ge.1.and.i.le.N_y) then
                        ...
                        !b_spr(row_cnt) = (1D0/delta_t) * (3 * u_tmp(i,j) - 4 * u_tmp(i,j-1) + u_tmp(i,j-2))
                        b_spr(row_cnt) = 0
                        cycle      
                  end if
                  ...
                  ! if within cylinder cells (walls and interior)
                  if(i.gt.18.and.i.lt.22.and.j.gt.51.and.j.lt.55) then    
                  ...
                  !write(6,*) "int cells",i,j, b_spr(row_cnt)
                  !b_spr(row_cnt) = row_cnt
...
subroutine convective_operator(u, v, N_u, N_v)
      ...
      ! v nodes from Row 2 to Row N_x, Column 1 to Column N_y
      do i = 2, N_y
            do j = 1, N_x
...
subroutine u_tmp_boundaries(u_tmp, p)
      ...
      do i = 1, N_y
        ...
        !call g_operator(p, i, 1, 1, G)
        !u_tmp(i,1) = u_inlet + delta_t * G
        u_tmp(i,1) = u_inlet
...
subroutine v_tmp_boundaries(v_tmp, p)
      ...
      do j = 1, N_x
            ...
            ! v* for the bottom wall
            !call g_operator(p, 1, j, 2, G_1)
            !v_tmp(1,j) = delta_t * G_1
            v_tmp(1,j) = 0

            ! v* for the top wall
            !call g_operator(p, N_y+1, j, 2, G_Nyp1)
            !v_tmp(N_y+1,j) = delta_t * G_Nyp1
            v_tmp(N_y+1,j) = 0
...
subroutine u_tmp_sq_cyl(u_tmp, p)
      ...
      double precision :: G_bot_int, G_bot_ext, G_top_int, G_top_ext

      integer :: i, j

      ! create temp velocity for the left and right cylinder wall u = 0
      !do i = sq_cyl_row_index_start, sq_cyl_row_index_start + N_y_sq_cyl - 1

      !      u_tmp(i,sq_cyl_col_index_start) = 0
      !      u_tmp(i,sq_cyl_col_index_start + N_x_sq_cyl + 1) = 0


      !      !write(6,*) i, sq_cyl_col_index_start, "left cyl wall U"
      !      !write(6,*) i,sq_cyl_col_index_start + N_x_sq_cyl, "right cyl wall U"
      !      
      !end do
      
      ! start at 19, end at 21
      ! left and right cylinder walls
      do i = sq_cyl_row_index_start+1, sq_cyl_row_index_start + N_y_sq_cyl - 2

            u_tmp(i,sq_cyl_col_index_start) = 0
            u_tmp(i,sq_cyl_col_index_start + N_x_sq_cyl) = 0
      ...
      ! u* for the top and bottom cylinder walls (u = 0)
      ! start at 53 end at 54
      do j = sq_cyl_col_index_start+2, sq_cyl_col_index_start + (N_x_sq_cyl - 2)

            ! create u* for the top cylinder wall
            call g_operator(p,  sq_cyl_row_index_start + N_y_sq_cyl - 1, j, 1, G_top_int)
            call g_operator(p,  sq_cyl_row_index_start + N_y_sq_cyl, j, 1, G_top_ext)
            u_tmp(sq_cyl_row_index_start + N_y_sq_cyl - 1,j) = -u_tmp(sq_cyl_row_index_start + N_y_sq_cyl,j) & 
                  + delta_t * (G_top_ext + G_top_int) 
            
            !u_tmp(sq_cyl_row_index_start + N_y_sq_cyl - 1,j) = -u_tmp(sq_cyl_row_index_start + N_y_sq_cyl,j) 

            !write(6,*) sq_cyl_row_index_start + N_y_sq_cyl - 1, j, "top interior cyl cell U"
            !write(6,*) sq_cyl_row_index_start + N_y_sq_cyl, j, "top exterior cyl cell U"

    
            ! create u* for the bot wall
            call g_operator(p, sq_cyl_row_index_start, j, 1, G_bot_int)
            call g_operator(p, sq_cyl_row_index_start - 1, j, 1, G_bot_ext)
            u_tmp(sq_cyl_row_index_start,j) = -u_tmp(sq_cyl_row_index_start - 1,j) + delta_t * (G_bot_int + G_bot_ext) 
            
            !u_tmp(sq_cyl_row_index_start,j) = -u_tmp(sq_cyl_row_index_start - 1,j)

            !write(6,*) sq_cyl_row_index_start, j, "bottom interior cyl cell U"
            !write(6,*) sq_cyl_row_index_start - 1, j, "bottom exterior cyl cell U"
    
      end do

      !if((i.eq.18.and.j.eq.(52.or.55)).or.(i.eq.22.and.j.eq.(52.or.55))) then
      if(i.eq.18.and.j.eq.52.or.i.eq.18.and.j.eq.55.or.i.eq.22.and.j.eq.52.or.i.eq.22.and.j.eq.55) then
      ...
      do i = sq_cyl_row_index_start + 1, sq_cyl_row_index_start + N_y_sq_cyl - 2
            do j = sq_cyl_col_index_start + 1, sq_cyl_col_index_start + N_x_sq_cyl - 1
                  
                  u_tmp(i,j) = 0
                  
                  !write(6,*) i, j, "interior cyl cell U"
            end do
      end do


end subroutine u_tmp_sq_cyl

subroutine v_tmp_sq_cyl(v_tmp, p)
      ...
      double precision, dimension(0:N_y+2,0:N_x+2), intent(in) :: p
      integer :: i, j
      double precision :: G_left_int, G_left_ext, G_right_ext, G_right_int

      ! create temp velocity for the top and bottom cylinder wall v = 0
      do j = sq_cyl_col_index_start + 1, sq_cyl_col_index_start + N_x_sq_cyl - 2

            v_tmp(sq_cyl_row_index_start,j) = 0
            v_tmp(sq_cyl_row_index_start + N_y_sq_cyl, j) = 0
            
            !write(6,*) sq_cyl_row_index_start, j, "bottom cyl wall V"
            !write(6,*) sq_cyl_row_index_start + N_y_sq_cyl, j, "top cyl wall V"
            
      end do

      ! v* for the left and right cylinder walls (v = 0)
      ! 50 & 51 from 20 to 21, 55 & 56 from 20 to 21
      do i = sq_cyl_row_index_start + 2, sq_cyl_row_index_start + (N_y_sq_cyl - 2)

            ! create v* for the left wall
            call g_operator(p, i, sq_cyl_col_index_start - 1, 2, G_left_ext)
            call g_operator(p, i, sq_cyl_col_index_start, 2, G_left_int)
            v_tmp(i,sq_cyl_col_index_start) = -v_tmp(i,sq_cyl_col_index_start-1) & 
                  + delta_t * (G_left_ext + G_left_int)
            !v_tmp(i,sq_cyl_col_index_start) = -v_tmp(i,sq_cyl_col_index_start-1)

            !write(6,*) i, sq_cyl_col_index_start - 1, "left cyl wall ext V"
            !write(6,*) i, sq_cyl_col_index_start, "left cyl wall int V"
    
            ! create v* for the right wall
            call g_operator(p, i, sq_cyl_col_index_start + N_x_sq_cyl - 1, 2, G_right_int)
            call g_operator(p, i, sq_cyl_col_index_start + N_x_sq_cyl, 2, G_right_ext)
            v_tmp(i, sq_cyl_col_index_start + N_x_sq_cyl - 1) = -v_tmp(i, sq_cyl_col_index_start + N_x_sq_cyl) & 
                  + delta_t * (G_right_int + G_right_ext) 
            
            !v_tmp(i, sq_cyl_col_index_start + N_x_sq_cyl - 1) = -v_tmp(i, sq_cyl_col_index_start + N_x_sq_cyl) 
            
            !write(6,*) i, sq_cyl_col_index_start + N_x_sq_cyl - 1, "right cyl wall int V"
            !write(6,*) i, sq_cyl_col_index_start + N_x_sq_cyl, "right cyl wall ext V"
    
      end do


      ! corner v*
      if(i.eq.19.and.j.eq.51.or.i.eq.19.and.j.eq.55.or.i.eq.22.and.j.eq.51.or.i.eq.22.and.j.eq.55) then

            v_tmp(i,j) = 0

      end if

      do i = sq_cyl_row_index_start + 1, sq_cyl_row_index_start + N_y_sq_cyl - 1
            do j = sq_cyl_col_index_start + 1, sq_cyl_col_index_start + N_x_sq_cyl - 2
                  v_tmp(i,j) = 0
                  !write(6,*) i, j, "interior cyl cell V"
            end do
      end do



end subroutine v_tmp_sq_cyl
